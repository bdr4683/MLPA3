...
def show_score( data, data_tuple, clf, show_setting=False ):
    ...
    for i in BNRS_CLASS:
        draw_contours(data, clf.predict_proba, "Metal Parts Scores for " +
                      ...
                      file_name = BNRS_CLASS[i] + "_" + str(clf.best_params_).replace(' ','_')+".pdf")
...
def main():
    ...
    # Plots
    draw_results( a2_data, grid_out.predict_proba, 'Q2 RF' + str(grid_out.best_params_), BNRS_CLASS_FORMAT)
    draw_results( a2_data, sdt.predict_proba, 'Q2 DTree ' + str(sdt), BNRS_CLASS_FORMAT)
